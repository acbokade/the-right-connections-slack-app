...
def search_messages(client, query):
    search_results = client.search_messages(token=os.environ.get("USER_TOKEN"), query=query)
    return search_results['messages']['matches']
        ...
        blocks=[
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": ":white_check_mark: Accepted request to invoke search command!",
                },
            }
        ],
    )
